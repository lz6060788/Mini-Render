好的，我们开始 **Stage 1: 数学工具与基础图形类** 的实现。
...
这是 ZRender 能够处理复杂层级和动画的核心。我们只需要实现最关键的几个方法：创建、乘法（级联）、合成（属性转矩阵）。
...
下一步，我们将构建 **Storage** (仓库) 和 **Painter** (渲染器)，将这些手动的步骤自动化，形成真正的渲染循环。准备好了吗？